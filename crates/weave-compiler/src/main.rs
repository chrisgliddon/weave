...
fn main() -> ExitCode {
    ...
    if cli.watch {
        ...
        ExitCode::from(compile_once(&cli))
    }
}

fn compile_once(cli: &Cli) -> u8 {
    let source = match fs::read_to_string(&cli.input) {
        Ok(source) => source,
        Err(error) => {
            eprintln!("weavec: could not read {}: {error}", cli.input.display());
            return 2;
        }
    };
    let options = CompileOptions {
        source_name: Some(cli.input.display().to_string()),
    };
    let compiled = match compile(&source, &options) {
        Ok(compiled) => compiled,
        Err(error) => {
            print_diagnostics(&cli.input, &source, &error.diagnostics);
            return 1;
        }
    };
    print_diagnostics(&cli.input, &source, &compiled.diagnostics);
...
fn watch(cli: &Cli) -> Result<(), String> {
    let input = absolute(&cli.input).map_err(|error| error.to_string())?;
    let parent = input
        .parent()
    ...
    let _ = compile_once(cli);
    eprintln!("watching {}", cli.input.display());
    loop {
        match receiver.recv() {
