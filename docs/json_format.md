...
The canonical schema for IR version 3 is available as a site [download](downloads/weave-story-ir-v3.schema.json). `weavec --schema` generates that document from the Rust IR types, and the compiler test suite verifies that the checked-in file is byte-for-byte current. The version-2 schema remains published only for consumers of historical artifacts.
...
- Explicitly activated [domain modules](domain_modules.md) are embedded under `modules`, keyed by story-local alias. Each entry records exact module and pack identities and tagged export values, so RON, Rust, JavaScript, Bevy, and browser consumers see the same finite data without a registry or editor dependency.
