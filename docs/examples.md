...
| Weave Character end to end | Complete six-factor/24-facet profile, recomputed read-only OCEAN projection, optional extensions, template overlay, deterministic two-character corpus operations, reference-safe rename review/apply, locked domain pack, source/editor/CLI parity, exact RON/JSON, Bevy, and PixiJS consumption | `cargo run -p weave-character -- collection-propose examples/domain-modules/weave-character/operations/collection.character-collection.json examples/domain-modules/weave-character/operations/rename.character-request.json --output target/rename.character-proposal.json` |
