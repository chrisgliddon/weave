...
| Freehack end to end | Verified release/commit-pinned CC0 source, campaign-defined characters, exact integer probability/resolution, public/secret generic tracks and memories, simultaneous sections, structurally separate authority/public artifacts, replay, and Rust/Bevy/PixiJS portability | `cargo run -p weave-tabletop --example freehack_fixture -- --check` |
