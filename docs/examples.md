...
| Weave Character end to end | Complete six-factor/24-facet profile, recomputed read-only OCEAN projection, optional extensions, template overlay, locked domain pack, source/editor round-trip, exact RON/JSON, Bevy, and PixiJS consumption | `cargo run -p weave-compiler -- examples/domain-modules/weave-character/ari-vale.weave --locked --format json --output target/ari-vale.story.json` |
