...
The [domain-module guide](domain_modules.md) explains the shared manifest, packaging, activation, editor reload, compiled IR, and consumer boundaries. The [third-party tutorial](domain_module_tutorial.md) builds a complete original module with public tools only, while the [Weave World guide](world_module.md) follows four openly redistributable reference-place seeds through the offline corpus, compiler, editor, runtime, RON, JSON, Bevy, and PixiJS boundaries.
