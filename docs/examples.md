...
| Weave Character end to end | Complete six-factor/24-facet profile, typed identity presentation, deterministic balanced catalog review/locks/override, recomputed read-only OCEAN, pluggable alignment, temporal context, exact RON/JSON, Bevy, and PixiJS consumption | `cargo run -p weave-character -- presentation-propose examples/domain-modules/weave-character/presentation/input.character-collection.json examples/domain-modules/weave-character/presentation/glasswind.presentation-catalog.json examples/domain-modules/weave-character/presentation/allocation.presentation-request.json --output target/proposal.presentation-proposal.json` |
