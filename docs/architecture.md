...
`weave-character` is another domain-specific contract above `weave-domain`. It validates profiles, evidence state, extension isolation, exact lineage, immutable template fingerprints, sparse operations, reproducible derived views, and deterministic collection proposals/reviews, then projects a validated profile into an ordinary declarative manifest and pack. The editor depends inward on this crate only for `CharacterCorpusSession`; it does not duplicate mutation rules. Compiler, Bevy, and PixiJS consumers continue to use declarative output. File persistence belongs to the finite CLI/editor host; the contract library remains independent of source parsing, provider, network, runtime, and engine APIs.
