...
`weave-character` is another domain-specific contract above `weave-domain`. It validates profiles, evidence state, extension isolation, exact lineage, immutable template fingerprints, sparse operations, and reproducible derived views, then projects a validated profile into an ordinary declarative manifest and pack. The compiler, editor, Bevy, and PixiJS consume that shared output rather than depending back on `weave-character`. File handling belongs to its finite CLI; the library remains independent of source parsing, editor, provider, network, runtime, and engine APIs.
