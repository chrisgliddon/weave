...
| `weave-character` | Versioned Character Profile/collection/guided-authoring schemas, canonical evidence boundaries, optional projections, original questionnaire scoring and conflict review, scoped/reviewed/resumable corpus operations, deterministic template synthesis, and validated domain-pack projection | `weave-domain` plus general-purpose CLI, hashing, schema, semantic-version, and serialization crates; never parser, compiler, runtime, editor, network, provider, or engine APIs |
...
`weave-character` is another domain-specific contract above `weave-domain`. It validates profiles, evidence state, extension isolation, exact lineage, immutable template fingerprints, sparse operations, reproducible derived views, guided workspaces, fixed-point questionnaire proposals, explicit conflict/final reviews, and deterministic collection proposals/reviews, then projects a validated profile into an ordinary declarative manifest and pack. The editor depends inward on this crate for `CharacterCorpusSession`, `CharacterAuthoringSession`, alignment, and temporal sessions; it does not duplicate mutation rules. Compiler, Bevy, and PixiJS consumers continue to use declarative output. File persistence belongs to the finite CLI/editor host; the contract library remains independent of source parsing, provider, network, runtime, and engine APIs.
