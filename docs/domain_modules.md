...
The synthetic fixture is original MIT-licensed material created for this contract. It does not model a real place, person, game, or setting and uses no external dataset. Domain-specific modules must carry equivalent machine-readable provenance for every bundled corpus or derived value. The [Weave World reference-seed guide](world_module.md) shows that boundary with hashed public sources, field-level transformation claims, editor inspection, and portable consumers. The [Weave Character guide](character_module.md) defines the corresponding authority boundary for canonical personality evidence, optional projections, suggestions, derived views, and template overlays.
